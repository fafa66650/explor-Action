...
const META = {
  ...
  'sorede': { icon:'🌿', color:'#3DD7C4', description:'Nature, four solaire et patrimoine local.' },
  'montesquieu': { icon:'🧭', color:'#FF6DB1', description:'Village, relief et lecture du terrain.' },
  'palau': { icon:'🪞', color:'#7AB8FF', description:'Verre, artisanat et patrimoine de village.' },
  'theza': { icon:'🔥', color:'#F67A45', description:'Village express, rythme local et repères de proximité.' }
};
const FUTURE_VILLAGES = [];
const AGE_PROFILES = {
  child: {
    label:'Enfant',
    distances:{ discover:2.8, explorer:4.2, expert:5.8 },
    durations:{ discover:[35,55], explorer:[50,75], expert:[65,95] },
    steps:{ discover:3, explorer:4, expert:5 },
    validations:{ discover:6, explorer:8, expert:10 },
    challenge:{ discover:35, explorer:45, expert:55 },
    deniveleFactor:.55,
    prompt:'Consignes plus simples, lecture plus directe, défis plus accessibles.'
  },
  teen: {
    label:'Ado',
    distances:{ discover:4.8, explorer:7.2, expert:9.6 },
    durations:{ discover:[45,70], explorer:[60,95], expert:[80,120] },
    steps:{ discover:4, explorer:5, expert:6 },
    validations:{ discover:8, explorer:10, expert:12 },
    challenge:{ discover:40, explorer:50, expert:60 },
    deniveleFactor:.8,
    prompt:'Rythme plus soutenu, observation plus fine, missions plus sportives.'
  },
  adult: {
    label:'Adulte',
    distances:{ discover:8.5, explorer:11.8, expert:14.6 },
    durations:{ discover:[95,150], explorer:[130,200], expert:[170,255] },
    steps:{ discover:5, explorer:6, expert:7 },
    validations:{ discover:10, explorer:12, expert:14 },
    challenge:{ discover:45, explorer:60, expert:75 },
    deniveleFactor:1,
    prompt:'Boucles plus longues, pression plus marquée, dénivelé et validations plus exigeants.'
  }
};
const TERRAIN_PROFILES = {
  'argeles': { denivele: 80, loops:'front de mer + cœur ancien' },
  'saint-andre': { denivele: 65, loops:'centre-bourg + liaison patrimoine' },
  'laroque': { denivele: 160, loops:'ruelles + château + montée' },
  'villelongue': { denivele: 220, loops:'village + balcon sur les Albères' },
  'collioure': { denivele: 180, loops:'port + montée patrimoine' },
  'port-vendres': { denivele: 140, loops:'quais + belvédère' },
  'banyuls': { denivele: 260, loops:'front de mer + vigne + relief' },
  'sorede': { denivele: 210, loops:'village + nature + four solaire' },
  'montesquieu': { denivele: 280, loops:'centre + pente + point de vue' },
  'palau': { denivele: 70, loops:'village + atelier + circulation douce' },
  'theza': { denivele: 55, loops:'centre + liaison rapide' }
};
...
function difficultyCfg(){
  const age = AGE_PROFILES[state.config.ageGroup] || AGE_PROFILES.adult;
  const key = state.config.difficulty;
  const base = {
    discover: { label:'Découverte', bonus:false, hintPenalty:6, errorPenalty:2, scoreBoost:0 },
    explorer: { label:'Explorateur', bonus:true, hintPenalty:10, errorPenalty:5, scoreBoost:4 },
    expert: { label:'Expert', bonus:true, hintPenalty:14, errorPenalty:8, scoreBoost:8 }
  }[key];
  return {
    ...base,
    steps: age.steps[key],
    distanceKm: age.distances[key],
    duration: age.durations[key],
    validations: age.validations[key],
    challengeTimer: age.challenge[key],
    ageLabel: age.label
  };
}
function currentAdaptiveCfg(){
  ...
  let scoreBoost = base.scoreBoost;
  let mode = 'Équilibré';
...
function routeSummary(village=currentVillage()){
  ...
  const terrain = TERRAIN_PROFILES[village.id || state.villageKey] || { denivele:100, loops:'boucle terrain' };
  const age = AGE_PROFILES[state.config.ageGroup] || AGE_PROFILES.adult;
  const deniv = Math.round(terrain.denivele * age.deniveleFactor * ({discover:1, explorer:1.15, expert:1.35}[state.config.difficulty] || 1));
  const extraStep = diff.bonus ? 1 : 0;
  return {
    duration:`${diff.duration[0]} à ${diff.duration[1]} min`,
    distance:`${String(diff.distanceKm).replace('.', ',')} km`,
    steps: diff.steps + extraStep,
    validations: diff.validations + (diff.bonus ? 2 : 0),
    mode: diff.mode,
    denivele:`${deniv} m D+`,
    loops: terrain.loops,
    ageLabel: diff.ageLabel
  };
}

function enrichStep(step, village, index){
  const ageKey = state.config.ageGroup;
  const ageLabel = AGE_PROFILES[ageKey].label;
  const diffLabel = difficultyCfg().label;
  const precision = {
    child: {
      story:'Version enfant : repère le lieu, écoute l’histoire, puis réponds avec des mots simples.',
      qStart:'Mission ',
      fieldStart:'Sur place, cherche bien : ',
      challenge:'Version enfant : défi court et accessible.'
    },
    teen: {
      story:'Version ado : utilise observation, mémoire et logique pour aller plus vite sans te tromper.',
      qStart:'Défi ado ',
      fieldStart:'Validation terrain ado : ',
      challenge:'Version ado : le rythme monte, garde de la précision.'
    },
    adult: {
      story:'Version adulte : lecture plus fine du terrain, boucle plus longue et validation plus exigeante.',
      qStart:'Analyse adulte ',
      fieldStart:'Validation adulte : ',
      challenge:'Version adulte : ajoute du déplacement, de la précision et une restitution plus complète.'
    }
  }[ageKey];
  const hardening = {
    discover: { intro:'Format découverte', suffix:'Reste fluide et lisible.', extraOptions:[] },
    explorer: { intro:'Format explorateur', suffix:'Observe davantage avant de trancher.', extraOptions:['Une réponse approximative mais incomplète'] },
    expert: { intro:'Format expert', suffix:'Cherche la réponse la plus précise, pas seulement la plus plausible.', extraOptions:['Une réponse séduisante mais fausse', 'Une option piège liée au mauvais contexte'] }
  }[state.config.difficulty];

  const qOptions = [...step.question.options];
  const fOptions = [...step.field.options];
  hardening.extraOptions.forEach(opt => { if(!qOptions.includes(opt)) qOptions.push(opt); if(!fOptions.includes(opt)) fOptions.push(opt); });

  return {
    ...step,
    story: `${precision.story} ${hardening.intro}. ${step.story}`,
    question: {
      ...step.question,
      prompt: `${precision.qStart}${index+1} · ${ageLabel} · ${diffLabel} : ${step.question.prompt} ${hardening.suffix}`,
      options: qOptions,
      hint: `${step.question.hint} (${ageLabel.toLowerCase()} / ${diffLabel.toLowerCase()})`,
      explanation: `${step.question.explanation} Niveau ${ageLabel.toLowerCase()} · ${diffLabel.toLowerCase()} validé.`
    },
    field: {
      ...step.field,
      prompt: `${precision.fieldStart}${step.field.prompt} ${hardening.suffix}`,
      options: fOptions,
      hint: `${step.field.hint} (${ageLabel.toLowerCase()} / ${diffLabel.toLowerCase()})`,
      explanation: `${step.field.explanation} Validation terrain ${ageLabel.toLowerCase()} réussie.`
    },
    challenge: {
      ...step.challenge,
      title: `${step.challenge.title} · ${ageLabel}`,
      instruction: `${precision.challenge} ${step.challenge.instruction} Difficulté ${diffLabel}.`,
      reward: `${step.challenge.reward} · profil ${ageLabel.toLowerCase()}`
    }
  };
}
...
function buildStepCoords(village){
  ...
  const shifts = [[0,0],[0.0038,-0.0044],[-0.0033,0.0041],[0.0052,0.0018],[-0.0048,-0.0026],[0.0028,0.0054],[-0.0051,0.0059],[0.0064,-0.0031],[0.0019,-0.006],[0.0071,0.0038]];
...
function getStepsForVillage(){
  ...
  let steps = village.steps.slice(0, Math.min(village.steps.length, diff.steps)).map((step,i)=>enrichStep({ ...step, coords: coords[i] || village.center }, village, i));
  while(steps.length < diff.steps){
    const baseIndex = steps.length % village.steps.length;
    const extraIndex = steps.length;
    const source = village.steps[baseIndex];
    const remix = enrichStep({
      ...structuredClone(source),
      id:`${source.id}-loop-${extraIndex}`,
      place:`${source.place} · boucle ${extraIndex - village.steps.length + 2}`,
      repere:`Boucle avancée : ${source.repere}`,
      story:`Boucle avancée du parcours. ${source.story}`,
      challenge:{
        ...source.challenge,
        title:`${source.challenge.title} · boucle`,
        instruction:`Variante longue du parcours : ${source.challenge.instruction}`,
        reward:`${source.challenge.reward} · bonus boucle`
      },
      coords: coords[extraIndex] || village.center
    }, village, extraIndex);
    steps.push(remix);
  }
  if(diff.bonus){
    steps.push({
      ...
      challenge:{
        type:'memoire',
        title:`Défi final immersion · ${(AGE_PROFILES[state.config.ageGroup]||AGE_PROFILES.adult).label}`,
        instruction:`Résume le village à ton équipe puis cite un lieu, un repère et une idée forte du parcours. Niveau ${difficultyCfg().label}.`,
...
function renderSetupSummaryChips(summary){
  return `<span class="chip">👥 ${summary.ageLabel}</span><span class="chip">⏱️ ${summary.duration}</span><span class="chip">🚶 ${summary.distance}</span><span class="chip">⛰️ ${summary.denivele}</span><span class="chip">🧩 ${summary.steps} étapes</span>`;
}
...
function refreshSetupMetrics(){
  ...
  if(boxes[2]) boxes[2].innerHTML = `<strong>${summary.ageLabel}</strong><span>Public</span>`;
  if(boxes[3]) boxes[3].innerHTML = `<strong>${summary.denivele}</strong><span>Dénivelé</span>`;
}

function setupChoiceButtons(){
  const modeTarget = $('#modeChoices'); const ageTarget = $('#ageChoices'); const diffTarget = $('#difficultyChoices'); const timingTarget = $('#timingChoices');
  const modes = [['solo','Solo'],['team','Équipe'],['multi','Multi‑équipes']];
  const ages = [['child','Enfant'],['teen','Ado'],['adult','Adulte']];
  ...
  ageTarget.innerHTML = ages.map(([v,l])=>`<button class="choice ${state.config.ageGroup===v?'active':''}" data-age="${v}">${l}</button>`).join('');
  ...
  $$('[data-age]').forEach(btn => btn.onclick = ()=>{ state.config.ageGroup = btn.dataset.age; setupChoiceButtons(); refreshSetupMetrics(); const helper = $('.helper-text'); if(helper) helper.textContent = AGE_PROFILES[state.config.ageGroup].prompt; });

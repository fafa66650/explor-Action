...
renderTerritoryHub = function(territoryId='banyuls'){
  ...
  const tp=territoryMissionProgress(territoryId),available=MISSIONS.filter(m=>m.territoryId===territoryId&&allowed(m)&&(missionPublished(m)||state.organizerTest)),filtered=available.filter(m=>state.settings.routeFilter==='all'||m.route.level===state.settings.routeFilter),hasFinale=!!v.finale&&tp.complete;

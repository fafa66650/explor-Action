...
const MISSION_GUIDE = {}; // V5.4.2: guide joueur neutre; l’identité visuelle privilégie l’exploration et les territoires.
...
async function verifyCode(code){
  try{
    ...
    if(!['5.4.0','5.4.1','5.4.2'].includes(payload.version)||payload.keyId!==ACTIVE_KEY_ID||!expiresAt||Date.now()>expiresAt||issuedAt>Date.now()+300000)return null;
...
function renderAccess(){
  ...
  app.innerHTML=`<main class="shell access-player exploration-access"><header class="simple-brand exploration-brand">${logoHTML(false,'access-logo')}<span><b>FAFATRAINING</b><small>EXPLORATION</small></span></header><section class="card hero access-immersive exploration-gate"><div><div class="eyebrow">FAFATRAINING EXPLORATION</div><h1 class="title">Explor’Action</h1><p class="access-promise">Territoires, patrimoine, orientation et énigmes. Le terrain réel devient ton aventure.</p><div id="deepStatus" class="deep-status ${dl.access?'':'hidden'}" role="status">Lien Explor’Action détecté • ouverture sécurisée…</div><div class="access-actions"><button class="btn big-cta qr-primary" id="scanQr">▣ Scanner mon QR</button><details class="access-method"><summary>🔗 J’ai reçu un lien d’accès</summary><label for="accessLink">Colle le lien reçu</label><input id="accessLink" class="input" inputmode="url" autocomplete="off" placeholder="https://…/explor-action/?access=…"><button class="btn secondary full-btn" id="openLink">Ouvrir cet accès</button></details><details class="access-method access-backup"><summary>⌨️ J’ai un code de secours</summary><p class="small">À utiliser seulement si le QR ou le lien ne fonctionne pas.</p><label for="accessCode">Code de secours</label><textarea id="accessCode" class="input code-input" rows="3" autocomplete="one-time-code" autocapitalize="off" spellcheck="false" placeholder="FAFA5.…">${escapeHtml(dl.access||'')}</textarea><button class="btn secondary full-btn" id="enter">Valider le code</button></details></div><p class="access-reassurance">🔒 Accès vérifié sur ton appareil • aucun compte joueur nécessaire</p><button class="text-link admin-entry" id="admin">Accès organisateur</button></div><div class="access-guide"><img class="access-avatar" src="${ASSETS.welcome}" alt="FAFA, guide d’exploration"><span class="compass-stamp" aria-hidden="true">⌖</span><p>FAFA t’accompagne. Le territoire reste le héros.</p></div></section></main>`;
  const run=async(target,button)=>{if(!target.access)return toast('Lien ou code non reconnu');if(button){button.disabled=true;button.textContent='Vérification…'}const r=await activatePlayerAccess(target.access,target);if(button){button.disabled=false;button.textContent=button.id==='enter'?'Valider le code':'Ouvrir cet accès'}if(!r.ok){if($('#deepStatus'))$('#deepStatus').textContent=r.message;toast(r.message)}};
  $('#scanQr').onclick=openQrScanner;
  $('#openLink').onclick=()=>run(accessTargetFromText($('#accessLink').value),$('#openLink'));
  $('#enter').onclick=()=>run(accessTargetFromText($('#accessCode').value),$('#enter'));
  $('#admin').onclick=renderAdminLogin;
  if(dl.access)setTimeout(()=>run(dl,null),80);
}
...
function renderHome(){
  ...
  app.innerHTML=`<main class="shell home-clean">${state.organizerTest?'<div class="test-banner">🧪 MODE TEST ORGANISATEUR — scores non officiels</div>':''}${utilityTop()}<section class="card hero clean-home-hero exploration-home"><div class="home-copy"><div class="eyebrow">FAFATRAINING • EXPLORATION</div><h1>Explor’Action</h1><p class="home-mantra">Explore. Observe. Résous. Découvre.</p><p class="subtitle">Des aventures territoriales où patrimoine, orientation, paysages et énigmes se rencontrent.</p>${resume?`<button class="btn big-cta" id="resumeHome">▶️ Reprendre ${escapeHtml(resume.title)}</button><button class="btn secondary" id="startHome">Explorer les territoires</button>`:`<button class="btn big-cta" id="startHome">Explorer les territoires</button>`}<div class="exploration-signature"><span>🧭 Orientation</span><span>🏛 Patrimoine</span><span>🗺 Territoires</span><span>🧩 Énigmes</span></div><div class="home-secondary"><button class="text-link" id="scores">Mon carnet d’aventures</button>${bp.done?`<span class="home-progress">${bp.done}/${bp.total} aventures accomplies</span>`:''}</div></div><img class="hero-avatar clean-avatar" src="${ASSETS.welcome}" alt="FAFA, guide d’exploration"></section></main>`;
...
function renderTerritories(){
  ...
  const territorySummary=v=>{const n=MISSIONS.filter(m=>m.territoryId===v.id&&missionPublished(m)&&allowed(m)).length,words=(v.themeWords||[]).slice(0,3).join(' • ');return `${n||'Des'} aventure${n>1?'s':''}${words?` • ${escapeHtml(words)}`:''}`};
  app.innerHTML=`<main class="shell">${utilityTop()}<section class="screen-intro slim-intro exploration-intro"><div><div class="eyebrow">TERRITOIRES À EXPLORER</div><h1>Choisis ton point de départ</h1><p class="subtitle">Chaque territoire possède sa propre ambiance, son histoire et plusieurs façons de l’explorer.</p></div></section><div class="territory-grid featured-territories exploration-territories">${ready.map(v=>`<button class="territory-player-card ready featured-territory territory-card-${v.id}" data-village="${v.id}">${territoryVisual(v,true)}<div class="territory-card-copy"><small>${v.kicker}</small><h2>${v.name}</h2><p>${territorySummary(v)}</p><b>Découvrir ce territoire →</b></div></button>`).join('')}</div><details class="coming-territories" ${state.organizerTest?'open':''}><summary>${soon.length} territoires en préparation${state.organizerTest?' • mode Test actif':''}</summary><div class="territory-grid soon-territory-grid">${soon.map(v=>{const n=MISSIONS.filter(m=>m.territoryId===v.id).length,canTest=state.organizerTest&&n>0;return `<button class="territory-player-card soon compact-soon ${canTest?'testable':''}" data-village="${v.id}"><span class="territory-icon">${v.icon}</span><div><small>${v.kicker}</small><h2>${v.name}</h2><b>${canTest?`🧪 Tester ${n} aventure${n>1?'s':''}`:'En préparation'}</b></div></button>`}).join('')}</div></details><button class="text-link back-link" id="back">← Accueil</button></main>`;
...
function missionCard(m){const p=prog(m),r=m.route,progress=pct(m,p),distance=r.playerDistance||r.distanceLabel||'À mesurer',ascent=r.playerAscent||'Dénivelé à contrôler',duration=r.gameTime||r.playerWalk||r.walkTime||'Durée à estimer';return `<button class="mission-player-card ${routeLevelClass(r.level)}" data-mid="${m.id}"><div class="mission-card-top"><span class="mission-icon">${m.icon}</span><b class="difficulty-chip ${routeLevelClass(r.level)}">${r.levelLabel}</b></div><small>${m.tag}</small>${m.expedition?'<span class="expedition-badge">EXPÉDITION</span>':''}${m.flatRoute?'<span class="flat-badge">FAIBLE DÉNIVELÉ</span>':''}<h3>${m.title}</h3><div class="mission-key-metrics"><span><b>${escapeHtml(distance)}</b><small>distance</small></span><span><b>${escapeHtml(ascent)}</b><small>dénivelé</small></span><span><b>${escapeHtml(duration)}</b><small>avec jeu</small></span></div><p>${m.teaser||m.story}</p><div class="card-bottom"><span>${progress?`${progress}% accompli`:'À découvrir'}</span><b>Voir l’aventure →</b></div></button>`}
...
renderAdventureDetail = function(mid){
  ...
  <section class="card hero adventure-detail exploration-adventure-detail"><div><div class="eyebrow">${escapeHtml(v?.name||'Explor’Action')} • EXPLORATION</div><div class="mission-kickers"><span class="pill">${m.icon} ${m.tag}</span><b class="difficulty-chip ${routeLevelClass(r.level)}">${r.levelLabel}</b></div><h1>${m.title}</h1><p class="adventure-teaser">${m.teaser}</p><div class="mission-key-metrics detail-metrics"><span><b>${escapeHtml(r.playerDistance||r.distanceLabel||'À contrôler')}</b><small>distance</small></span><span><b>${escapeHtml(r.playerAscent||'À contrôler')}</b><small>dénivelé</small></span><span><b>${escapeHtml(r.gameTime||'À estimer')}</b><small>avec jeu</small></span></div><button class="btn big-cta" id="choose">Préparer le départ</button><button class="text-link adventure-settings-link" id="customize">Modifier équipe / style : ${escapeHtml(state.settings.public==='enfant'?'Enfant / Famille':state.settings.public==='adulte'?'Adulte':'Ado')}</button></div><img class="hero-avatar adventure-guide-avatar" src="${ASSETS[MISSION_GUIDE[m.id]||'welcome']}" alt="FAFA, guide d’exploration"></section>
  <section class="detail-grid compact-details"><article class="card"><h2>Ce que tu vas chercher</h2><div class="objective-box"><small>OBJECTIF D’EXPLORATION</small><b>${m.objective}</b></div><p class="small">Le briefing complet s’ouvre juste avant le départ pour garder la découverte intacte.</p>${scenarioDetails(m.scenarioDisclaimer)}</article>
  <article class="card"><h2>Le parcours en bref</h2><dl class="terrain-dl">${publicRouteRows(r)}</dl><p class="small terrain-context">${r.playerNote||''}</p></article></section>
  ...
  ${p.started?`<section class="card resume-strip"><div><b>Aventure déjà commencée</b><span>${p.done.length}/${m.steps.length} étapes</span></div><button class="btn secondary" id="resumeDirect">Reprendre</button></section>`:''}</main>`;
...
renderPreflight = async function(mid){
  ...
  app.innerHTML=`<main class="shell">${state.organizerTest?'<div class="test-banner">🧪 MODE TEST ORGANISATEUR</div>':''}<header class="topbar exploration-topbar">${logoHTML()}<button class="btn secondary" id="back">← Aventure</button></header>
  <section class="card briefing-player exploration-briefing"><div class="briefing-copy"><div class="eyebrow">BRIEFING D’EXPLORATION</div><h1>${m.title}</h1><p class="briefing-story">${m.briefing}</p><div class="objective-box"><small>TA MISSION</small><b>${m.objective}</b></div>${scenarioDetails(m.scenarioDisclaimer)}</div><img src="${ASSETS[MISSION_GUIDE[m.id]||'welcome']}" alt="FAFA, guide d’exploration"></section>
  <section class="card departure-card"><div class="departure-main"><div><div class="eyebrow">PRÊT À EXPLORER ?</div><h2>${p.started?'Reprendre là où tu t’es arrêté':'Vérifie l’essentiel puis pars'}</h2><div class="mission-key-metrics departure-metrics"><span><b>${escapeHtml(r.playerDistance||r.distanceLabel||'À contrôler')}</b><small>distance</small></span><span><b>${escapeHtml(r.playerAscent||'À contrôler')}</b><small>dénivelé</small></span><span><b>${escapeHtml(r.gameTime||'À estimer')}</b><small>avec jeu</small></span></div></div><div class="checklist quick-checklist"><div id="chkGps">⏳ GPS</div><div id="chkOffline">⏳ Hors connexion</div><div id="chkBattery">⏳ Batterie</div></div></div><div class="departure-actions"><button class="btn secondary" id="runChecks">↻ Revérifier</button><button class="btn big-cta" id="goMission">${p.started?'▶️ Reprendre':'🧭 Commencer l’exploration'}</button></div><p class="small team-summary">${escapeHtml(state.team.name||'Équipe sans nom')} • ${state.team.players} joueur${state.team.players>1?'s':''} • ${state.settings.public==='enfant'?'Enfant / Famille':state.settings.public==='adulte'?'Adulte':'Ado'}</p></section>
  <details class="card departure-details" ${m.expedition?'open':''}><summary>Terrain, sécurité et préparation${m.expedition?' • Expédition':''}</summary><div class="departure-detail-grid"><div><h3>Terrain</h3><p>🥾 ${r.playerTerrain||r.surface}</p><p>♿ ${r.accessibility}</p>${hasTrail?'<p class="trail-note">🟨 Sur les portions de randonnée, le balisage et les indications officielles passent avant l’écran.</p>':''}</div><div><h3>Sécurité</h3><p>${m.safety}</p><a class="btn danger" href="${FIRE_RISK_URL}" target="_blank" rel="noopener">🔥 Risque incendie du jour ↗</a></div></div>${m.expedition?`<div class="warning expedition-essentials"><b>Expédition :</b> eau suffisante, nourriture, chaussures adaptées, protection solaire, vêtements adaptés au vent/météo, téléphone chargé, batterie externe et contenu hors connexion. Choisis une heure de départ compatible avec la durée.</div>`:''}</details>
  ${guideHTML('guard','Le terrain passe avant le jeu','Si une zone est fermée, déviée ou dangereuse, arrête-toi et utilise SOS. FAFA te guide, mais les consignes locales et le balisage passent toujours en premier.')}
...
renderMission = function(tab='mission'){
  ...
  if(p.paused){app.innerHTML=`<main class="shell"><header class="topbar">${logoHTML()}<button class="btn secondary" id="quit">← Accueil</button></header><section class="card hero pause-exploration"><img class="hero-avatar" src="${ASSETS.welcome}" alt="FAFA, guide d’exploration"><div><span class="pill">⏸ EXPLORATION EN PAUSE</span><h1>${m.title}</h1><p class="subtitle">Ta progression est enregistrée. Reprends quand le terrain et le groupe sont prêts.</p><p>${p.done.length}/${m.steps.length} étapes • <b>${formatMs(liveElapsed(p))}</b></p><button class="btn" id="resume">▶️ Reprendre</button></div></section></main>`;$('#quit').onclick=renderHome;$('#resume').onclick=()=>{resumeClock(p);renderMission()};return}
  app.innerHTML=`<main class="shell mission-shell">${state.organizerTest?'<div class="test-banner compact">🧪 TEST</div>':''}<header class="mission-head explorer-mission-head"><div class="mission-head-copy"><small>${escapeHtml(territory?.name||'Explor’Action')} • ${m.icon} exploration</small><b class="mission-title-mini">${m.title}</b><div class="mission-progress"><i style="width:${pct(m,p)}%"></i></div><span class="mission-mini-status">${p.done.length}/${m.steps.length} étapes • <span id="clock">${formatMs(liveElapsed(p))}</span></span></div><button class="icon-btn" id="pause" aria-label="Mettre en pause">⏸</button><button class="icon-btn" id="home" aria-label="Accueil">⌂</button></header><nav class="mission-nav"><button data-tab="mission">🎯 Mission</button><button data-tab="map">🗺 Carte</button><button data-tab="bag">📖 Journal</button><button data-tab="help">🆘 Aide</button></nav><section id="view"></section></main>`;
...
renderMapTab = function(v,m,p){
  ...
  v.innerHTML=`<section class="card"><div class="section-head"><div><h2>Carte terrain hors connexion</h2><p class="small">Anti-spoiler : seulement les points GPS déjà découverts et le prochain repère. Le tracé relie uniquement des points connus ; il ne remplace jamais le sentier réel.</p></div><span class="pill">${pct(m,p)}%</span></div>${gpsVis.length?'<div id="offlineMissionMap" class="offline-map-shell"></div>':'<div class="schematic-note">🥾 Cette portion repose sur des repères terrain : suis le balisage et les indications de Mission.</div>'}${hasTrail?'<div class="schematic-note">🟨 Les étapes sentier ne sont pas transformées en faux points GPS.</div>':''}<div class="row map-actions">${gpsVis.length?'<button class="btn secondary" id="mapgps">📍 Actualiser ma position</button>':''}<a class="btn secondary" href="https://www.openstreetmap.org/" target="_blank" rel="noopener">Carte en ligne ↗</a></div><div class="map-list">${m.steps.map((s,i)=>vis.includes(i)?`<button class="map-row ${p.done.includes(i)?'done':'current'}" data-step="${i}"><span>${p.done.includes(i)?'✓':i+1}</span><div><b>${p.done.includes(i)?escapeHtml(s.name):'Prochaine découverte'}</b><small>${p.done.includes(i)?'Terminée':s.locationMode==='trail'?'Repère à trouver sur l’itinéraire':'Nom révélé sur place'}</small></div></button>`:`<div class="map-row lock"><span>🔒</span><div><b>Étape à découvrir</b><small>Nom masqué</small></div></div>`).join('')}</div></section>`;
...
renderHelp = function(v,m,p){
  ...
  v.innerHTML=`<section class="card"><h2>🆘 Aide terrain</h2>${guideHTML('guard','Une aventure, jamais un risque','Ne cours jamais pour gagner des points et ne coupe jamais un sentier. Le balisage, les fermetures et la sécurité passent avant le jeu.')}<div class="help-grid"><div><h3>GPS</h3><button class="btn secondary" id="testGps">Tester mon GPS</button><p id="gpsRes" class="small"></p></div><div><h3>Audio</h3><button class="btn secondary" id="sound">${state.settings.sound?'🔊 Activé':'🔇 Coupé'}</button></div><div><h3>Hors connexion</h3><p class="small">${navigator.serviceWorker?.controller?'✅ Jeu, données et carte terrain locale sont disponibles hors connexion.':'⚠️ Ouvre l’aventure une fois en ligne avant le départ.'}</p></div></div>${trail?'<p class="trail-note">🟨 Sur le sentier, utilise le balisage et les indications officielles. Une étape « repère » n’est pas une invitation à quitter l’itinéraire.</p>':''}<a class="btn danger" href="${FIRE_RISK_URL}" target="_blank" rel="noopener">🔥 Risque incendie du jour ↗</a><details><summary>Règles de sécurité</summary><p>${m.safety}</p></details><details><summary>Barème de l’équipe</summary><p class="small">Score : ${p.score} pts • ${p.hints} indice(s) • ${p.errors} erreur(s) • ${p.skipped.length} SOS. Le score ne justifie jamais de courir, de traverser une propriété ou de quitter un sentier.</p></details></section>`;
